...
//This seems to be a dry-run tester of the flashing/event behaviour?
	...
	IEnumerator SingleFlashCor() {
		...
		while(startFlashes) {
			...
			if(flash_counter[randomCube] > 0) {
				...
				//Handle events if this is the target cube or not
		...
		p300_Controller.WriteMarker("P300 SingleFlash Ends");
		startFlashes = !startFlashes;
		p300_Controller.LockKeysToggle(KeyCode.S);
